...
Unit* CreatureEventAI::DoSelectLowestHpFriendly(float range, uint32 MinHPDiff)
{
    ...
    Strawberry::MostHPMissingInRangeCheck u_check(m_creature, range, MinHPDiff);
    Strawberry::UnitLastSearcher<Strawberry::MostHPMissingInRangeCheck> searcher(pUnit, u_check);
...
{
    Strawberry::FriendlyCCedInRangeCheck u_check(m_creature, range);
    Strawberry::CreatureListSearcher<Strawberry::FriendlyCCedInRangeCheck> searcher(_list, u_check);
    Cell::VisitGridObjects(m_creature, searcher, range);
}

void CreatureEventAI::DoFindFriendlyMissingBuff(std::list<Creature*>& _list, float range, uint32 spellid)
{
    Strawberry::FriendlyMissingBuffInRangeCheck u_check(m_creature, range, spellid);
    Strawberry::CreatureListSearcher<Strawberry::FriendlyMissingBuffInRangeCheck> searcher(_list, u_check);
    Cell::VisitGridObjects(m_creature,searcher, range);
}
