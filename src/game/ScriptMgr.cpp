    ...
    do
    {
        ...
        switch(tmp.command)
        {
            case SCRIPT_COMMAND_TALK:
            {
                ...
                // if (!GetStrawberryStringLocale(tmp.dataint)) will be checked after db_script_string loading
                break;
            }
            ...
            case SCRIPT_COMMAND_TELEPORT_TO:
            {
                ...
                if (!Strawberry::IsValidMapCoord(tmp.x, tmp.y, tmp.z, tmp.o))
                {
...
{
    sObjectMgr.LoadStrawberryStrings(WorldDatabase, "db_script_string", MIN_DB_SCRIPT_STRING_ID, MAX_DB_SCRIPT_STRING_ID);

    std::set<int32> ids;

    for(int32 i = MIN_DB_SCRIPT_STRING_ID; i < MAX_DB_SCRIPT_STRING_ID; ++i)
        if (sObjectMgr.GetStrawberryStringLocale(i))
            ids.insert(i);
    ...
    for(ScriptMapMap::const_iterator itrMM = scripts.begin(); itrMM != scripts.end(); ++itrMM)
    {
        for(ScriptMap::const_iterator itrM = itrMM->second.begin(); itrM != itrMM->second.end(); ++itrM)
        {
            if (itrM->second.command == SCRIPT_COMMAND_TALK)
            {
                for(int i = 0; i < MAX_TEXT_ID; ++i)
                {
                    if (itrM->second.talk.textId[i] && !sObjectMgr.GetStrawberryStringLocale(itrM->second.talk.textId[i]))
...
ScriptLoadResult ScriptMgr::LoadScriptLibrary(const char* libName)
{
    ...
    name = STRAWBERRY_SCRIPT_PREFIX + name + STRAWBERRY_SCRIPT_SUFFIX;

    m_hScriptLib = STRAWBERRY_LOAD_LIBRARY(name.c_str());
...
#   define GET_SCRIPT_HOOK_PTR(P,N)             \
    ...
    // let check used StrawberryWorld revision for build library (unsafe use with different revision because changes in inline functions, define and etc)
    char const* (STRAWBERRY_IMPORT* pGetStrawberryRevStr) ();

    GET_SCRIPT_HOOK_PTR(pGetStrawberryRevStr,              "GetStrawberryRevStr");
...
#   undef GET_SCRIPT_HOOK_PTR
    ...
    if (strcmp(pGetStrawberryRevStr(), REVISION_NR) != 0)
    {
...
void ScriptMgr::UnloadScriptLibrary()
{
    ...
    STRAWBERRY_CLOSE_LIBRARY(m_hScriptLib);
    m_hScriptLib = NULL;
