...
namespace Strawberry
{
    ...
    class BattleGround2ChatBuilder
    {
        public:
            ...
            {
                char const* text = sObjectMgr.GetStrawberryString(i_textId,loc_idx);
                char const* arg1str = i_arg1 ? sObjectMgr.GetStrawberryString(i_arg1,loc_idx) : "";
                char const* arg2str = i_arg2 ? sObjectMgr.GetStrawberryString(i_arg2,loc_idx) : "";
...
uint32 BattleGround::GetBonusHonorFromKill(uint32 kills) const
{
    ...
    return (uint32)Strawberry::Honor::hk_honor_at_level(GetMaxLevel(), kills);
}
...
void BattleGround::SendRewardMarkByMail(Player *plr,uint32 mark, uint32 count)
{
    ...
    if (Item* markItem = Item::CreateItem(mark,count,plr))
    {
        ...
        // text
        std::string textFormat = plr->GetSession()->GetStrawberryString(LANG_BG_MARK_BY_MAIL);
        char textBuf[300];
...
void BattleGround::SendYellToAll(int32 entry, uint32 language, ObjectGuid guid)
{
    ...
    if(!source)
        return;
    Strawberry::BattleGroundYellBuilder bg_builder(language, entry, source);
    Strawberry::LocalizedPacketDo<Strawberry::BattleGroundYellBuilder> bg_do(bg_builder);
    BroadcastWorker(bg_do);
}
...
void BattleGround::SendYell2ToAll(int32 entry, uint32 language, ObjectGuid guid, int32 arg1, int32 arg2)
{
    ...
    if(!source)
        return;
    Strawberry::BattleGround2YellBuilder bg_builder(language, entry, source, arg1, arg2);
    Strawberry::LocalizedPacketDo<Strawberry::BattleGround2YellBuilder> bg_do(bg_builder);
    BroadcastWorker(bg_do);
}
