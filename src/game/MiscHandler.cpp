...
void WorldSession::HandleWhoOpcode( WorldPacket & recv_data )
{
    ...
    // client send in case not set max level value 100 but strawberry support 255 max level,
...
void WorldSession::HandleAddFriendOpcode( WorldPacket & recv_data )
{
    ...
    std::string friendName = GetStrawberryString(LANG_FRIEND_IGNORE_UNKNOWN);
...
void WorldSession::HandleAddIgnoreOpcode( WorldPacket & recv_data )
{
    ...
    std::string IgnoreName = GetStrawberryString(LANG_FRIEND_IGNORE_UNKNOWN);
    ...
    if (!pl->isGameMaster())
    {
        ...
        if (missingItem || missingLevel || missingQuest)
        {
            ...
            // hack for TBC heroics
                ...
                SendAreaTriggerMessage(GetStrawberryString(LANG_LEVEL_MINREQUIRED), at->requiredLevel);
            else
...
void WorldSession::HandleMoveTimeSkippedOpcode( WorldPacket & recv_data )
{
    ...
    /*
        ...
        /// TODO
        must be need use in strawberry
...
void WorldSession::HandleRealmSplitState( WorldPacket & recv_data )
{
    DEBUG_LOG("CMSG_REALM_SPLIT_STATE");
