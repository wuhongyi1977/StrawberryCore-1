...
ChatCommand * ChatHandler::getCommandTable()
{
    ...
    static ChatCommand reloadCommandTable[] =
    {
        ...
        { "strawberry_string",               SEC_ADMINISTRATOR, true,  &ChatHandler::HandleReloadStrawberryStringCommand,            "", NULL },
...
{
    return value ?  GetStrawberryString(LANG_ON) : GetStrawberryString(LANG_OFF);
}
...
void ChatHandler::SendSysMessage(const char *str)
{
    ...
    char* buf = strawberry_strdup(str);
    char* pos = buf;
...
std::string ChatHandler::PrepareStringNpcOrGoSpawnInformation(uint32 guid)
{
    ...
    if (uint16 pool_id = sPoolMgr.IsPartOfAPool<T>(guid))
    {
        ...
        {
            char buffer[100];
            const char* format = GetStrawberryString(LANG_NPC_GO_INFO_POOL_EVENT_STRING);
        ...
        {
            char buffer[100];
            const char* format = GetStrawberryString(LANG_NPC_GO_INFO_POOL_STRING);
    ...
    {
        char buffer[100];
        const char* format = GetStrawberryString(LANG_NPC_GO_INFO_EVENT_STRING);
