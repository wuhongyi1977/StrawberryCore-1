...
/*void Object::BuildMovementUpdate(ByteBuffer * data, uint16 flags) const
{
    *data << uint16(flags);                           // update flags
    ...
    if (flags & UPDATEFLAG_LIVING)
    {
        ...
        unit->m_movementInfo.BuildMovementPacket(data);
...
void Object::BuildValuesUpdate(uint8 updatetype, ByteBuffer * data, UpdateMask *updateMask, Player *target) const
{
    ...
    STRAWBERRY_ASSERT(updateMask && updateMask->GetCount() == m_valuesCount);
...
{
    STRAWBERRY_ASSERT( index < m_valuesCount || PrintIndexError( index, true ) );
...
{
    STRAWBERRY_ASSERT( index + 1 < m_valuesCount || PrintIndexError( index, true ) );
...
{
    return Strawberry::IsValidMapCoord(m_position.x,m_position.y,m_position.z,m_position.o);
}
...
void WorldObject::MonsterSay(int32 textId, uint32 language, Unit* target)
{
    ...
    Strawberry::MonsterChatBuilder say_build(*this, CHAT_MSG_MONSTER_SAY, textId, language, target);
    Strawberry::LocalizedPacketDo<Strawberry::MonsterChatBuilder> say_do(say_build);
    Strawberry::CameraDistWorker<Strawberry::LocalizedPacketDo<Strawberry::MonsterChatBuilder> > say_worker(this, range, say_do);
...
void WorldObject::MonsterYell(int32 textId, uint32 language, Unit* target)
{
    ...
    Strawberry::MonsterChatBuilder say_build(*this, CHAT_MSG_MONSTER_YELL, textId, language, target);
    Strawberry::LocalizedPacketDo<Strawberry::MonsterChatBuilder> say_do(say_build);
    Strawberry::CameraDistWorker<Strawberry::LocalizedPacketDo<Strawberry::MonsterChatBuilder> > say_worker(this,range,say_do);
    Cell::VisitWorldObjects(this, say_worker, range);
}
...
void WorldObject::MonsterTextEmote(int32 textId, Unit* target, bool IsBossEmote)
{
    ...
    Strawberry::MonsterChatBuilder say_build(*this, IsBossEmote ? CHAT_MSG_RAID_BOSS_EMOTE : CHAT_MSG_MONSTER_EMOTE, textId, LANG_UNIVERSAL, target);
    Strawberry::LocalizedPacketDo<Strawberry::MonsterChatBuilder> say_do(say_build);
    Strawberry::CameraDistWorker<Strawberry::LocalizedPacketDo<Strawberry::MonsterChatBuilder> > say_worker(this,range,say_do);
    Cell::VisitWorldObjects(this, say_worker, range);
}
...
void WorldObject::SendMessageToSetExcept(WorldPacket *data, Player const* skipped_receiver)
{
    ...
    if (IsInWorld())
    {
        Strawberry::MessageDelivererExcept notifier(this, data, skipped_receiver);
