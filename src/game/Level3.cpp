...
bool ChatHandler::HandleAHBotItemsAmountCommand(char* args)
{
    ...
    for (int i = 0; i < MAX_AUCTION_QUALITY; ++i)
        PSendSysMessage(LANG_AHBOT_ITEMS_AMOUNT, GetStrawberryString(ahbotQualityIds[i]), sAuctionBotConfig.getConfigItemQualityAmount(AuctionQuality(i)));
...
bool ChatHandler::HandleAHBotItemsAmountQualityCommand(char* args)
{
    ...
    PSendSysMessage(LANG_AHBOT_ITEMS_AMOUNT, GetStrawberryString(ahbotQualityIds[Q]),
...
bool ChatHandler::HandleAHBotStatusCommand(char* args)
{
    ...
    PSendSysMessage(fmtId, GetStrawberryString(LANG_AHBOT_STATUS_ITEM_COUNT),
    ...
    if (all)
    {
        PSendSysMessage(fmtId, GetStrawberryString(LANG_AHBOT_STATUS_ITEM_RATIO),
        ...
        for (int i = 0; i < MAX_AUCTION_QUALITY; ++i)
            PSendSysMessage(fmtId, GetStrawberryString(ahbotQualityIds[i]),
...
bool ChatHandler::HandleReloadAllCommand(char* /*args*/)
{
    ...
    HandleReloadStrawberryStringCommand((char*)"");
...
bool ChatHandler::HandleReloadStrawberryStringCommand(char* /*args*/)
{
    sLog.outString( "Re-Loading strawberry_string Table!" );
    sObjectMgr.LoadStrawberryStrings();
    SendGlobalSysMessage("DB table `strawberry_string` reloaded.");
    return true;
}
...
void ChatHandler::ShowAchievementCriteriaListHelper(AchievementCriteriaEntry const* criEntry, AchievementEntry const * achEntry, LocaleConstant loc, Player* target /*= NULL*/)
{
    ...
    if (achEntry->flags & ACHIEVEMENT_FLAG_COUNTER)
        ss << GetStrawberryString(LANG_COUNTER);
    else
    {
        ...
        if (target && target->GetAchievementMgr().IsCompletedCriteria(criEntry, achEntry))
            ss << GetStrawberryString(LANG_COMPLETE);
    }
...
bool ChatHandler::HandleAddItemCommand(char* args)
{
    ...
    DETAIL_LOG(GetStrawberryString(LANG_ADDITEM), itemId, count);
...
bool ChatHandler::HandleAddItemSetCommand(char* args)
{
    ...
    DETAIL_LOG(GetStrawberryString(LANG_ADDITEMSET), itemsetId);
...
void ChatHandler::ShowItemListHelper( uint32 itemId, int loc_idx, Player* target /*=NULL*/ )
{
    ...
    if (target)
    {
        if (target->CanUseItem(itemProto))
            usableStr = GetStrawberryString(LANG_COMMAND_ITEM_USABLE);
    }
...
bool ChatHandler::HandleBanInfoHelper(uint32 accountid, char const* accountname)
{
    ...
    do
    {
        ...
        PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
            fields[0].GetString(), bantime.c_str(), active ? GetStrawberryString(LANG_BANINFO_YES):GetStrawberryString(LANG_BANINFO_NO), fields[4].GetString(), fields[5].GetString());
    }while (result->NextRow());
...
bool ChatHandler::HandleBanInfoIPCommand(char* args)
{
    ...
    PSendSysMessage(LANG_BANINFO_IPENTRY,
        fields[0].GetString(), fields[1].GetString(), permanent ? GetStrawberryString(LANG_BANINFO_NEVER):fields[2].GetString(),
        permanent ? GetStrawberryString(LANG_BANINFO_INFINITE):secsToTimeString(fields[3].GetUInt64(), true).c_str(), fields[4].GetString(), fields[5].GetString());
    delete result;
    return true;
}
...
bool ChatHandler::HandleMmapTestArea(char* args)
{
    ...
    CellPair pair(Strawberry::ComputeCellPair( m_session->GetPlayer()->GetPositionX(), m_session->GetPlayer()->GetPositionY()) );
    Cell cell(pair);
    cell.SetNoCreate();

    std::list<Creature*> creatureList;

    Strawberry::AnyUnitInObjectRangeCheck go_check(m_session->GetPlayer(), radius);
    Strawberry::CreatureListSearcher<Strawberry::AnyUnitInObjectRangeCheck> go_search(creatureList, go_check);
