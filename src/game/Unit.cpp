...
Unit::~Unit()
{
    ...
    STRAWBERRY_ASSERT(m_gameObj.size() == 0);
    STRAWBERRY_ASSERT(m_dynObjGUIDs.size() == 0);
    STRAWBERRY_ASSERT(m_deletedAuras.size() == 0);
    STRAWBERRY_ASSERT(m_deletedHolders.size() == 0);
}
...
void Unit::SendHeartBeat()
{
    ...
    m_movementInfo.BuildMovementPacket(&data);
}
...
uint32 Unit::DealDamage(Unit *pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const *spellProto, bool durabilityLoss)
{
    ...
    if (health <= damage)
    {
        ...
        if(duel_hasEnded)
        {
            STRAWBERRY_ASSERT(pVictim->GetTypeId()==TYPEID_PLAYER);
            Player *he = (Player*)pVictim;

            STRAWBERRY_ASSERT(he->duel);
...
{
    STRAWBERRY_ASSERT(pSpell);                                  // NULL may be never passed here, use InterruptSpell or InterruptNonMeleeSpells
...
{
    STRAWBERRY_ASSERT(spellType < CURRENT_MAX_SPELL);
...
{
    STRAWBERRY_ASSERT(gameObj && !gameObj->GetOwnerGuid());
...
{
    STRAWBERRY_ASSERT(gameObj && gameObj->GetOwnerGuid() == GetObjectGuid());
...
{
    STRAWBERRY_ASSERT(GetTypeId() == TYPEID_UNIT);
...
Unit* Unit::SelectRandomUnfriendlyTarget(Unit* except /*= NULL*/, float radius /*= ATTACK_DISTANCE*/) const
{
    ...
    Strawberry::AnyUnfriendlyUnitInObjectRangeCheck u_check(this, this, radius);
    Strawberry::UnitListSearcher<Strawberry::AnyUnfriendlyUnitInObjectRangeCheck> searcher(targets, u_check);
...
Unit* Unit::SelectRandomFriendlyTarget(Unit* except /*= NULL*/, float radius /*= ATTACK_DISTANCE*/) const
{
    ...
    Strawberry::AnyFriendlyUnitInObjectRangeCheck u_check(this, radius);
...
bool Unit::SetPosition(float x, float y, float z, float orientation, bool teleport)
{
    ...
    if (!Strawberry::IsValidMapCoord(x, y, z, orientation))
    {
...
void MovementInfo::BuildMovementPacket(ByteBuffer *data) const
{
    ...
    bool swimming = ((GetMovementFlags() & (MOVEFLAG_SWIMMING | MOVEFLAG_FLYING))
        || (moveFlags2 & MOVEFLAG2_ALLOW_PITCHING));
    bool interPolatedTurning = moveFlags2 & MOVEFLAG2_INTERP_TURNING;
    bool jumping = GetMovementFlags() & MOVEFLAG_FALLING;
    bool _splineElevation = GetMovementFlags() & MOVEFLAG_SPLINE_ELEVATION;
    bool splineData = false;

    data->WriteBits(GetMovementFlags(), 30);
    ...
    data->WriteBit(_splineElevation);
...
public:
    ...
    bool Execute(uint64 /*e_time*/, uint32 /*p_time*/)
    {
        ...
        {
            Strawberry::PlayerRelocationNotifier notify((Player&)m_owner);
            Cell::VisitAllObjects(&m_owner,notify,radius);
        }
        else //if(m_owner.GetTypeId() == TYPEID_UNIT)
        {
            Strawberry::CreatureRelocationNotifier notify((Creature&)m_owner);
            Cell::VisitAllObjects(&m_owner,notify,radius);
        }
