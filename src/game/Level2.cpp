...
void ChatHandler::ShowTriggerTargetListHelper( uint32 id, AreaTrigger const* at, bool subpart /*= false*/ )
{
    if (m_session)
    {
        ...
        if(!subpart)
        {
            ...
            snprintf(dist_buf, 50, GetStrawberryString(LANG_TRIGGER_DIST), dist);
        }
        else
            dist_buf[0] = '\0';
...
void ChatHandler::ShowTriggerListHelper( AreaTriggerEntry const * atEntry )
{
    ...
    char const* tavern = sObjectMgr.IsTavernAreaTrigger(atEntry->id) ? GetStrawberryString(LANG_TRIGGER_TAVERN) : "";
    char const* quest = sObjectMgr.GetQuestForAreaTrigger(atEntry->id) ? GetStrawberryString(LANG_TRIGGER_QUEST) : "";
...
bool ChatHandler::HandleGameObjectAddCommand(char* args)
{
    ...
    DEBUG_LOG(GetStrawberryString(LANG_GAMEOBJECT_CURRENT), gInfo->name, db_lowGUID, x, y, z, o);
...
bool ChatHandler::HandleModifyRepCommand(char* args)
{
    ...
    if (!ExtractInt32(&args, amount))
    {
        ...
        {
            std::string rank = GetStrawberryString(ReputationRankStrIndex[r]);
            if (rank.empty())
                continue;
...
bool ChatHandler::HandlePInfoCommand(char* args)
{
    ...
    PSendSysMessage(LANG_PINFO_ACCOUNT, (target?"":GetStrawberryString(LANG_OFFLINE)), nameLink.c_str(), target_guid.GetCounter(), username.c_str(), accId, security, last_ip.c_str(), last_login.c_str(), latency);
...
void ChatHandler::ShowTicket(GMTicket const* ticket)
{
    ...
    if (!sObjectMgr.GetPlayerNameByGUID(ticket->GetPlayerGuid(), name))
        name = GetStrawberryString(LANG_UNKNOWN);
...
bool ChatHandler::HandleWpModifyCommand(char* args)
{
    ...
    if (!waypointInfo || waypointInfo->GetHighGuid() != HIGHGUID_UNIT)
        return false;                                       // must exist as normal creature in strawberry.sql 'creature_template'
...
bool ChatHandler::HandleWpShowCommand(char* args)
{
    ...
    if (show == "info")
    {
        ...
        do
        {
            ...
            for(int i = 0;  i < MAX_WAYPOINT_TEXT; ++i)
                PSendSysMessage(LANG_WAYPOINT_INFO_TEXT, i+1, textid[i], (textid[i] ? GetStrawberryString(textid[i]) : ""));
...
bool ChatHandler::HandleLookupEventCommand(char* args)
{
    ...
    for(uint32 id = 1; id < events.size(); ++id)
    {
        ...
        {
            char const* active = sGameEventMgr.IsActiveEvent(id) ? GetStrawberryString(LANG_ACTIVE) : "";
...
bool ChatHandler::HandleEventListCommand(char* args)
{
    ...
    char const* inactive = GetStrawberryString(LANG_FACTION_INACTIVE);
    char const* state = "";
...
bool ChatHandler::HandleEventInfoCommand(char* args)
{
    ...
    char const* activeStr = sGameEventMgr.IsActiveEvent(event_id) ? GetStrawberryString(LANG_ACTIVE) : "";
...
bool ChatHandler::HandlePoolInfoCommand(char* args)
{
    ...
    std::string active_str = GetStrawberryString(LANG_ACTIVE);
...
bool ChatHandler::HandleCharacterTitlesCommand(char* args)
{
    ...
    LocaleConstant loc = GetSessionDbcLocale();
    char const* targetName = target->GetName();
    char const* knownStr = GetStrawberryString(LANG_KNOWN);
