...
void CreatureEventAIMgr::LoadCreatureEventAI_Texts(bool check_entry_use)
{
    ...
    // Load EventAI Text
    sObjectMgr.LoadStrawberryStrings(WorldDatabase,"creature_ai_texts",MIN_CREATURE_AI_TEXT_STRING_ID,MAX_CREATURE_AI_TEXT_STRING_ID);
...
void CreatureEventAIMgr::LoadCreatureEventAI_Summons(bool check_entry_use)
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (!Strawberry::IsValidMapCoord(temp.position_x,temp.position_y,temp.position_z,temp.orientation))
            {
