        ...
        if (auction->owner)
        {
            ...
            if (plWName.empty())
            {
                ...
                if (!sObjectMgr.GetPlayerNameByGUID(ObjectGuid(HIGHGUID_PLAYER, auction->owner), plName))
                    plName = sObjectMgr.GetStrawberryStringForDBCLocale(LANG_UNKNOWN);
...
{
    STRAWBERRY_ASSERT(it);
    STRAWBERRY_ASSERT(mAitems.find(it->GetGUIDLow()) == mAitems.end());
    ...
    for (AuctionEntryMap::iterator itr = AuctionsMap.begin(); itr != AuctionsMap.end(); )
    {
        if (itr->second->moneyDeliveryTime)                 // pending auction
        {
            if (curTime > itr->second->moneyDeliveryTime)
            {
                ...
                STRAWBERRY_ASSERT(!itr->second->itemGuidLow);   // already removed or send in mail at won
                delete itr->second;
