...
bool Map::Add(Player *player)
{
    ...
    CellPair p = Strawberry::ComputeCellPair(player->GetPositionX(), player->GetPositionY());
    Cell cell(p);
...
Map::Add(T *obj)
{
    ...
    CellPair p = Strawberry::ComputeCellPair(obj->GetPositionX(), obj->GetPositionY());
...
void Map::MessageBroadcast(Player *player, WorldPacket *msg, bool to_self)
{
    ...
    Strawberry::MessageDeliverer post_man(*player, msg, to_self);
    TypeContainerVisitor<Strawberry::MessageDeliverer, WorldTypeMapContainer > message(post_man);
...
void Map::MessageBroadcast(WorldObject *obj, WorldPacket *msg)
{
    ...
    Strawberry::ObjectMessageDeliverer post_man(*obj,msg);
    TypeContainerVisitor<Strawberry::ObjectMessageDeliverer, WorldTypeMapContainer > message(post_man);
...
void Map::MessageDistBroadcast(Player *player, WorldPacket *msg, float dist, bool to_self, bool own_team_only)
{
    ...
    Strawberry::MessageDistDeliverer post_man(*player, msg, dist, to_self, own_team_only);
    TypeContainerVisitor<Strawberry::MessageDistDeliverer , WorldTypeMapContainer > message(post_man);
...
void Map::MessageDistBroadcast(WorldObject *obj, WorldPacket *msg, float dist)
{
    ...
    Strawberry::ObjectMessageDistDeliverer post_man(*obj, msg, dist);
    TypeContainerVisitor<Strawberry::ObjectMessageDistDeliverer, WorldTypeMapContainer > message(post_man);
...
void Map::Update(const uint32 &t_diff)
{
    ...
    Strawberry::ObjectUpdater updater(t_diff);
    // for creature
    TypeContainerVisitor<Strawberry::ObjectUpdater, GridTypeMapContainer  > grid_object_update(updater);
    // for pets
    TypeContainerVisitor<Strawberry::ObjectUpdater, WorldTypeMapContainer > world_object_update(updater);
    ...
    if (!IsBattleGroundOrArena())
    {
        for (GridRefManager<NGridType>::iterator i = GridRefManager<NGridType>::begin(); i != GridRefManager<NGridType>::end(); )
        {
            ...
            STRAWBERRY_ASSERT(grid->GetGridState() >= 0 && grid->GetGridState() < MAX_GRID_STATE);
...
{
    STRAWBERRY_ASSERT(player);

    CellPair old_val = Strawberry::ComputeCellPair(player->GetPositionX(), player->GetPositionY());
    CellPair new_val = Strawberry::ComputeCellPair(x, y);
...
{
    STRAWBERRY_ASSERT(CheckGridIntegrity(creature,false));

    Cell old_cell = creature->GetCurrentCell();
    Cell new_cell(Strawberry::ComputeCellPair(x, y));
    ...
    STRAWBERRY_ASSERT(CheckGridIntegrity(creature,true));
}
...
bool Map::CheckGridIntegrity(Creature* c, bool moved) const
{
    ...
    CellPair xy_val = Strawberry::ComputeCellPair(c->GetPositionX(), c->GetPositionY());
    Cell xy_cell(xy_val);
    if(xy_cell != cur_cell)
    {
...
{
    cell.SetNoCreate();
    Strawberry::VisibleChangesNotifier notifier(*obj);
    TypeContainerVisitor<Strawberry::VisibleChangesNotifier, WorldTypeMapContainer > player_notifier(notifier);
...
{
    STRAWBERRY_ASSERT(obj->GetMapId()==GetId() && obj->GetInstanceId()==GetInstanceId());
...
{
    STRAWBERRY_ASSERT(x < MAX_NUMBER_OF_GRIDS);
    STRAWBERRY_ASSERT(y < MAX_NUMBER_OF_GRIDS);
    ...
    for(MapRefManager::const_iterator iter = m_mapRefManager.begin(); iter != m_mapRefManager.end(); ++iter)
    {
        ...
        CellPair p = Strawberry::ComputeCellPair(plr->GetPositionX(), plr->GetPositionY());
...
void Map::AddToActive( WorldObject* obj )
{
    ...
    Cell cell = Cell(Strawberry::ComputeCellPair(obj->GetPositionX(), obj->GetPositionY()));
    EnsureGridLoaded(cell);
    ...
    if (obj->GetTypeId() == TYPEID_UNIT)
    {
        ...
        if (!c->IsPet() && c->HasStaticDBSpawnData())
        {
            ...
            GridPair p = Strawberry::ComputeGridPair(x, y);
            if(getNGrid(p.x_coord, p.y_coord))
                getNGrid(p.x_coord, p.y_coord)->incUnloadActiveLock();
            else
            {
                GridPair p2 = Strawberry::ComputeGridPair(c->GetPositionX(), c->GetPositionY());
...
{
    STRAWBERRY_ASSERT(i_mapEntry->IsDungeon());
    ...
    else
    {
        ...
        else
        {
            ...
            if(!playerBind)
                ...
                // cannot jump to a different instance without resetting it
                STRAWBERRY_ASSERT(playerBind->state == GetPersistentState());
        }
    }
    ...
    while (!m_scriptSchedule.empty() && (iter->first <= sWorld.GetGameTime()))
    {
        ...
        switch(step.script->command)
        {
            case SCRIPT_COMMAND_TALK:
            {
                ...
                {
                    Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*pSource, step.script->talk.creatureEntry, true, step.script->talk.searchRadius);
                    Strawberry::CreatureLastSearcher<Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pBuddy, u_check);
            ...
            case SCRIPT_COMMAND_EMOTE:
            {
                ...
                {
                    Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*pSource, step.script->emote.creatureEntry, true, step.script->emote.searchRadius);
                    Strawberry::CreatureLastSearcher<Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pBuddy, u_check);
            ...
            case SCRIPT_COMMAND_MOVEMENT:
            {
                ...
                {
                    Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*pSource, step.script->movement.creatureEntry, true, step.script->movement.searchRadius);
                    Strawberry::CreatureLastSearcher<Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pMover, u_check);
            ...
            case SCRIPT_COMMAND_SET_ACTIVEOBJECT:
            {
                ...
                {
                    Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*pSource, step.script->activeObject.creatureEntry, true, step.script->activeObject.searchRadius);
                    Strawberry::CreatureLastSearcher<Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pOwner, u_check);
            ...
            case SCRIPT_COMMAND_SET_FACTION:
            {
                ...
                {
                    Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*pSource, step.script->faction.creatureEntry, true, step.script->faction.searchRadius);
                    Strawberry::CreatureLastSearcher<Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pOwner, u_check);
            ...
            case SCRIPT_COMMAND_MORPH_TO_ENTRY_OR_MODEL:
            {
                ...
                {
                    Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*pSource, step.script->morph.creatureEntry, true, step.script->morph.searchRadius);
                    Strawberry::CreatureLastSearcher<Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pOwner, u_check);
            ...
            case SCRIPT_COMMAND_MOUNT_TO_ENTRY_OR_MODEL:
            {
                ...
                {
                    Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*pSource, step.script->mount.creatureEntry, true, step.script->mount.searchRadius);
                    Strawberry::CreatureLastSearcher<Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pOwner, u_check);
            ...
            case SCRIPT_COMMAND_SET_RUN:
            {
                ...
                {
                    Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*pSource, step.script->run.creatureEntry, true, step.script->run.searchRadius);
                    Strawberry::CreatureLastSearcher<Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pOwner, u_check);
            ...
            case SCRIPT_COMMAND_ATTACK_START:
            {
                ...
                {
                    Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*pSource, step.script->attack.creatureEntry, true, step.script->attack.searchRadius);
                    Strawberry::CreatureLastSearcher<Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pBuddy, u_check);
            ...
            case SCRIPT_COMMAND_GO_LOCK_STATE:
            {
                ...
                Strawberry::NearestGameObjectEntryInObjectRangeCheck u_check(*pSearcher, step.script->goLockState.goEntry, step.script->goLockState.searchRadius);
                Strawberry::GameObjectLastSearcher<Strawberry::NearestGameObjectEntryInObjectRangeCheck> searcher(pGo, u_check);
            ...
            case SCRIPT_COMMAND_STAND_STATE:
            {
                ...
                {
                    Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*pSource, step.script->standState.creatureEntry, true, step.script->standState.searchRadius);
                    Strawberry::CreatureLastSearcher<Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pBuddy, u_check);
            ...
            case SCRIPT_COMMAND_MODIFY_NPC_FLAGS:
            {
                ...
                {
                    Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*pSource, step.script->npcFlag.creatureEntry, true, step.script->npcFlag.searchRadius);
                    Strawberry::CreatureLastSearcher<Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pBuddy, u_check);
...
void Map::MonsterYellToMap(CreatureInfo const* cinfo, int32 textId, uint32 language, Unit* target, uint32 senderLowGuid /*= 0*/)
{
    ...
    Strawberry::LocalizedPacketDo<StaticMonsterChatBuilder> say_do(say_build);
