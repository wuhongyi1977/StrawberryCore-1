...
{
    STRAWBERRY_ASSERT(m_RecvWPct == NULL);
    ...
    {
        sLog.outError("WorldSocket::handle_input_header: client sent malformed packet size = %d, cmd = %d", header.size, header.cmd);
    ...
    {
        STRAWBERRY_ASSERT(m_RecvPct.space() == 0);
    }
...
int WorldSocket::handle_input_payload(void)
{
    ...
    STRAWBERRY_ASSERT(m_RecvPct.space() == 0);
    STRAWBERRY_ASSERT(m_Header.space() == 0);
    STRAWBERRY_ASSERT(m_RecvWPct != NULL);
    ...
    while (message_block.length() > 0)
    {
        if (m_Header.space() > 0)
        {
            ...
            if (m_Header.space() > 0)
            {
                ...
                STRAWBERRY_ASSERT(message_block.length() == 0);
                errno = EWOULDBLOCK;
                return -1;
            }
            ...
            {
                STRAWBERRY_ASSERT((errno != EWOULDBLOCK) && (errno != EAGAIN));
                return -1;
            }
        }
...
int WorldSocket::ProcessIncoming(WorldPacket* new_pct)
{
    ...
    try
    {
        Opcodes opcodeLoading = LookupOpcodeEnum(opcode);
...
int WorldSocket::HandleAuthSession(WorldPacket& recvPacket)
{
    ...
    // Get the account information from the StrawberryRealm database
