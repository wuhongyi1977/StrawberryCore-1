...
{
    int accepted_versions[] = EXPECTED_STRAWBERRYWORLD_CLIENT_BUILD;
...
inline void LoadDBC(LocalData& localeData, BarGoLink& bar, StoreProblemList& errlist, DBCStorage<T>& storage, const std::string& dbc_path, const std::string& filename)
{
    ...
    STRAWBERRY_ASSERT(DBCFileLoader::GetFormatRecordSize(storage.GetFormat()) == sizeof(T) || LoadDBC_assert_print(DBCFileLoader::GetFormatRecordSize(storage.GetFormat()),sizeof(T),filename));
...
void LoadDBCStores(const std::string& dataPath)
{
    ...
    if (!sAreaStore.LookupEntry(4559)              ||       // last area (areaflag) added in 4.2.0
        !sCharTitlesStore.LookupEntry(279)         ||       // last char title added in 4.2.0
        !sGemPropertiesStore.LookupEntry(1860)     ||       // last gem property added in 4.2.0
        !sMapStore.LookupEntry(968)                ||       // last map added in 4.2.0
        !sSpellStore.LookupEntry(102129)           )        // last added spell in 4.2.0
    {
