...
void SpellCastTargets::read( ByteBuffer& data, Unit *caster )
{
    ...
    if (m_targetMask & TARGET_FLAG_SOURCE_LOCATION)
    {
        ...
        if(!Strawberry::IsValidMapCoord(m_srcX, m_srcY, m_srcZ))
    ...
    if (m_targetMask & TARGET_FLAG_DEST_LOCATION)
    {
        ...
        if(!Strawberry::IsValidMapCoord(m_destX, m_destY, m_destZ))
            throw ByteBufferException(false, data.rpos(), 0, data.size());
    }
...
WorldObject* Spell::FindCorpseUsing()
{
    ...
    Strawberry::WorldObjectSearcher<T> searcher(result, u_check);
    ...
    for(int i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        switch(spellEffect->EffectImplicitTargetA)
        {
            ...
            case TARGET_EFFECT_SELECT:
            {
                ...
                uint32 targetType = spellEffect->EffectImplicitTargetB;
                if (!targetType)
                    targetType = spellEffect->EffectImplicitTargetA;
...
void Spell::SetTargetMap(SpellEffectIndex effIndex, uint32 targetMode, UnitList& targetUnitMap)
{
    ...
    switch(targetMode)
    {
        ...
        case TARGET_RANDOM_ENEMY_CHAIN_IN_AREA:
        {
            ...
            {
                Strawberry::AnyAoETargetUnitInObjectRangeCheck u_check(m_caster, max_range);
                Strawberry::UnitListSearcher<Strawberry::AnyAoETargetUnitInObjectRangeCheck> searcher(tempTargetUnitMap, u_check);
        ...
        case TARGET_RANDOM_FRIEND_CHAIN_IN_AREA:
        {
            ...
            UnitList tempTargetUnitMap;
            {
                Strawberry::AnyFriendlyUnitInObjectRangeCheck u_check(m_caster, max_range);
                Strawberry::UnitListSearcher<Strawberry::AnyFriendlyUnitInObjectRangeCheck> searcher(tempTargetUnitMap, u_check);
                Cell::VisitAllObjects(m_caster, searcher, max_range);
            }
        ...
        case TARGET_CHAIN_DAMAGE:
        {
            ...
            else
            {
                ...
                UnitList tempTargetUnitMap;
                {
                    Strawberry::AnyAoEVisibleTargetUnitInObjectRangeCheck u_check(pUnitTarget, originalCaster, max_range);
                    Strawberry::UnitListSearcher<Strawberry::AnyAoEVisibleTargetUnitInObjectRangeCheck> searcher(tempTargetUnitMap, u_check);
                    Cell::VisitAllObjects(m_caster, searcher, max_range);
                }
                if (tempTargetUnitMap.empty())
                    break;
        ...
        case TARGET_AREAEFFECT_GO_AROUND_DEST:
        {
            ...
            for(SpellScriptTarget::const_iterator i_spellST = bounds.first; i_spellST != bounds.second; ++i_spellST)
            {
                if (i_spellST->second.type == SPELL_TARGET_TYPE_GAMEOBJECT)
                {
                    ...
                    Strawberry::GameObjectEntryInPosRangeCheck go_check(*m_caster, i_spellST->second.targetEntry, m_targets.m_destX, m_targets.m_destY, m_targets.m_destZ, radius);
                    Strawberry::GameObjectListSearcher<Strawberry::GameObjectEntryInPosRangeCheck> checker(tempTargetGOList, go_check);
            ...
            switch(spellEffect->Effect)
            {
                case SPELL_EFFECT_DUMMY:
                {
                    switch(m_spellInfo->Id)
                    {
                        ...
                        {
                            WorldObject* result = FindCorpseUsing<Strawberry::CannibalizeObjectCheck> ();
...
void Spell::SendCastResult(Player* caster, SpellEntry const* spellInfo, uint8 cast_count, SpellCastResult result)
{
    ...
    switch (result)
    {
        ...
        case SPELL_FAILED_REQUIRES_SPELL_FOCUS:
            data << uint32(spellInfo->GetRequiresSpellFocus());  // SpellFocusObject.dbc id
            break;
        ...
        case SPELL_FAILED_TOTEMS:
            {
                ...
                for(int i = 0; i < MAX_SPELL_TOTEMS; ++i)
                    if(totems && totems->Totem[i])
                        data << uint32(totems->Totem[i]);    // client needs only one id, not 2...
            }
            break;
        case SPELL_FAILED_TOTEM_CATEGORY:
            {
                ...
                for(int i = 0; i < MAX_SPELL_TOTEM_CATEGORIES; ++i)
                    if(totems && totems->TotemCategory[i])
                        data << uint32(totems->TotemCategory[i]);// client needs only one id, not 2...
            }
            break;
                ...
                for(SpellScriptTarget::const_iterator i_spellST = bounds.first; i_spellST != bounds.second; ++i_spellST)
                {
                    switch(i_spellST->second.type)
                    {
                        case SPELL_TARGET_TYPE_GAMEOBJECT:
                        {
                            ...
                            {
                                Strawberry::NearestGameObjectEntryInObjectRangeCheck go_check(*m_caster, i_spellST->second.targetEntry, range);
                                Strawberry::GameObjectLastSearcher<Strawberry::NearestGameObjectEntryInObjectRangeCheck> checker(p_GameObject, go_check);
                        ...
                        default:
                        {
                            ...
                            if (!targetExplicit)
                            {
                                Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*m_caster, i_spellST->second.targetEntry, i_spellST->second.type != SPELL_TARGET_TYPE_DEAD, range);
                                Strawberry::CreatureLastSearcher<Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(p_Creature, u_check);
        ...
        if(_target)                                         //for target dead/target not valid
        {
            ...
            else
            {
                ...
                for(int j = 0; j < MAX_EFFECT_INDEX; ++j)
                {
                    ...
                    if (m_spellInfo->GetEffectImplicitTargetAByIndex(SpellEffectIndex(j)) == TARGET_DUELVSPLAYER)
                    {
                        duelvsplayertar = true;
                        dualEffect = true;
                        break;
                    }
                    if (m_spellInfo->GetEffectImplicitTargetAByIndex(SpellEffectIndex(j)) == TARGET_IN_FRONT_OF_CASTER_30
                        || m_spellInfo->GetEffectImplicitTargetAByIndex(SpellEffectIndex(j)) == TARGET_MASTER
                        || m_spellInfo->GetEffectImplicitTargetAByIndex(SpellEffectIndex(j)) == TARGET_IN_FRONT_OF_CASTER
                        || m_spellInfo->GetEffectImplicitTargetAByIndex(SpellEffectIndex(j)) == TARGET_EFFECT_SELECT
                        || m_spellInfo->GetEffectImplicitTargetAByIndex(SpellEffectIndex(j)) == TARGET_CHAIN_DAMAGE
                        || m_spellInfo->GetEffectImplicitTargetAByIndex(SpellEffectIndex(j)) == TARGET_CASTER_COORDINATES)
                    {
                        dualEffect = true;
                        break;
                    }
...
SpellCastResult Spell::CheckItems()
{
    ...
    if(uint32 spellFocus = m_spellInfo->GetRequiresSpellFocus())
    {
        ...
        Strawberry::GameObjectFocusCheck go_check(m_caster, m_spellInfo->GetRequiresSpellFocus());
        Strawberry::GameObjectSearcher<Strawberry::GameObjectFocusCheck> checker(ok, go_check);
...
{
    Strawberry::SpellNotifierCreatureAndPlayer notifier(*this, targetUnitMap, radius, pushType, spellTargets, originalCaster);
