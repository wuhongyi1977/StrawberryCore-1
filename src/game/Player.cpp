...
// helper function, mainly for script side, but can be used for simple task in strawberry also.
...
void Player::Regenerate(Powers power, uint32 diff)
{
    ...
    switch (power)
    {
        case POWER_MANA:
        {
            ...
            if (recentCast)
            {
                // Strawberry Updates Mana in intervals of 2s, which is correct
    ...
    SetPower(Powers(GetPowerIndexByClass(power, getClass())), curValue);
}
...
void Player::UpdateCombatSkills(Unit *pVictim, WeaponAttackType attType, bool defence)
{
    ...
    uint32 greylevel = Strawberry::XP::GetGrayLevel(plevel);
...
int32 Player::CalculateReputationGain(ReputationSource source, int32 rep, int32 faction, uint32 creatureOrQuestLevel, bool noAuraBonus)
{
    ...
    if (rate != 1.0f && creatureOrQuestLevel <= Strawberry::XP::GetGrayLevel(getLevel()))
        percent *= rate;
...
bool Player::RewardHonor(Unit *uVictim, uint32 groupsize, float honor)
{
    ...
    if (honor <= 0)
    {
        ...
        if (uVictim->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            k_grey = Strawberry::XP::GetGrayLevel(k_level);
...
void Player::AddQuest( Quest const *pQuest, Object *questGiver )
{
    ...
    STRAWBERRY_ASSERT(log_slot < MAX_QUEST_LOG_SIZE);
    ...
    for(Quest::PrevQuests::const_iterator iter = qInfo->prevQuests.begin(); iter != qInfo->prevQuests.end(); ++iter)
    {
        ...
        if (qPrevInfo && i_prevstatus != mQuestStatus.end())
        {
            ...
            if (*iter > 0 && i_prevstatus->second.m_rewarded)
            {
                ...
                STRAWBERRY_ASSERT(bounds.first != bounds.second); // always must be found if qPrevInfo->ExclusiveGroup != 0
...
bool Player::SatisfyQuestExclusiveGroup(Quest const* qInfo, bool msg) const
{
    ...
    STRAWBERRY_ASSERT(bounds.first != bounds.second);           // must always be found if qInfo->ExclusiveGroup != 0
...
//not used in StrawberryWorld, function for external script library
...
{
    STRAWBERRY_ASSERT(count < 65536 && "mob/GO count store in 16 bits 2^16 = 65536 (0..65536)");
...
bool Player::LoadFromDB(ObjectGuid guid, SqlQueryHolder *holder )
{
    ...
    if(uint32 node_id = m_taxi.GetTaxiSource())
    {
        ...
        STRAWBERRY_ASSERT(nodeEntry);                           // checked in m_taxi.LoadTaxiDestinationsFromString
...
void Player::_LoadInventory(QueryResult *result, uint32 timediff)
{
    ...
    if (result)
    {
        ...
        {
            std::string subject = GetSession()->GetStrawberryString(LANG_NOT_EQUIPPED_ITEM);
...
void Player::HandleStealthedUnitsDetection()
{
    ...
    Strawberry::AnyStealthedCheck u_check(this);
    Strawberry::UnitListSearcher<Strawberry::AnyStealthedCheck > searcher(stealthedUnits, u_check);
...
{
    uint32 v_level = pVictim->getLevel();
    uint32 k_grey  = Strawberry::XP::GetGrayLevel(getLevel());
...
void Player::RewardSinglePlayerAtKill(Unit* pVictim)
{
    ...
    uint32 xp = PvP ? 0 : Strawberry::XP::Gain(this, pVictim);
