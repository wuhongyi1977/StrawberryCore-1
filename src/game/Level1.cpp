...
bool ChatHandler::HandleNotifyCommand(char* args)
{
    ...
    std::string str = GetStrawberryString(LANG_GLOBAL_NOTIFY);
    str += args;
...
bool ChatHandler::HandleGMVisibleCommand(char* args)
{
    if (!*args)
    {
        PSendSysMessage(LANG_YOU_ARE, m_session->GetPlayer()->isGMVisible() ?  GetStrawberryString(LANG_VISIBLE) : GetStrawberryString(LANG_INVISIBLE));
        return true;
    }
...
bool ChatHandler::HandleGPSCommand(char* args)
{
    ...
    DEBUG_LOG("Player %s GPS call for %s '%s' (%s: %u):",
        m_session ? GetNameLink().c_str() : GetStrawberryString(LANG_CONSOLE_COMMAND),
        (obj->GetTypeId() == TYPEID_PLAYER ? "player" : "creature"), obj->GetName(),
        (obj->GetTypeId() == TYPEID_PLAYER ? "GUID" : "Entry"), (obj->GetTypeId() == TYPEID_PLAYER ? obj->GetGUIDLow(): obj->GetEntry()) );

    DEBUG_LOG(GetStrawberryString(LANG_MAP_POSITION),
...
bool ChatHandler::HandleNamegoCommand(char* args)
{
    ...
    else
    {
        ...
        PSendSysMessage(LANG_SUMMONING, nameLink.c_str(),GetStrawberryString(LANG_OFFLINE));
...
bool ChatHandler::HandleModifyEnergyCommand(char* args)
{
    ...
    DETAIL_LOG(GetStrawberryString(LANG_CURRENT_ENERGY),chr->GetMaxPower(POWER_ENERGY));
...
bool ChatHandler::HandleModifyMoneyCommand(char* args)
{
    ...
    if (addmoney < 0)
    {
        ...
        DETAIL_LOG(GetStrawberryString(LANG_CURRENT_MONEY), moneyuser, addmoney, newmoney);
        if (newmoney <= 0 )
        {
    ...
    DETAIL_LOG(GetStrawberryString(LANG_NEW_MONEY), moneyuser, addmoney, chr->GetMoney() );
...
bool ChatHandler::HandleTeleNameCommand(char* args)
{
    ...
    else
    {
        ...
        PSendSysMessage(LANG_TELEPORTING_TO, nameLink.c_str(), GetStrawberryString(LANG_OFFLINE), tele->name.c_str());
