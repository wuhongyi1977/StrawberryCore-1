...
void WorldSession::HandlePetitionShowSignOpcode(WorldPacket & recv_data)
{
    ...
    data << uint32(petitionguid_low);                       // guild guid (in strawberry always same as GUID_LOPART(petitionguid)
...
void WorldSession::HandleOfferPetitionOpcode(WorldPacket & recv_data)
{
    ...
    data << uint32(petitionGuid.GetCounter());              // guild guid (in strawberry always same as low part of petition guid)
    data << uint8(signs);                                   // sign's count
