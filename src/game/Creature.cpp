...
void Creature::DoFleeToGetAssistance()
{
    ...
    if (radius >0)
    {
        ...
        Strawberry::CreatureLastSearcher<Strawberry::NearestAssistCreatureInCreatureRangeCheck> searcher(pCreature, u_check);
...
bool Creature::IsVisibleInGridForPlayer(Player* pl) const
{
    ...
    if (GetCreatureInfo()->type_flags[0] & CREATURE_TYPEFLAGS_GHOST_VISIBLE)
        return true;
    ...
    if( !m_AlreadyCallAssistance && getVictim() && !IsPet() && !isCharmed())
    {
        ...
        if(radius > 0)
        {
...

            {
                ...
                Strawberry::CreatureListSearcher<Strawberry::AnyAssistCreatureInRangeCheck> searcher(assistList, u_check);
                Cell::VisitGridObjects(this,searcher, radius);
            }
...
void Creature::GetRespawnCoord( float &x, float &y, float &z, float* ori, float* dist ) const
{
    ...
    STRAWBERRY_ASSERT(Strawberry::IsValidMapCoord(x, y, z) || PrintCoordinatesError(x, y, z, "respawn"));
}
