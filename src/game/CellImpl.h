...
#ifndef STRAWBERRY_CELLIMPL_H
#define STRAWBERRY_CELLIMPL_H
...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellPair center = Strawberry::ComputeCellPair(x, y).normalize();
        return CellArea(center, center);
    }

    return CellArea
    (
        Strawberry::ComputeCellPair(x - radius, y - radius).normalize(),
        Strawberry::ComputeCellPair(x + radius, y + radius).normalize()
    );
}
...
{
    CellPair p(Strawberry::ComputeCellPair(center_obj->GetPositionX(), center_obj->GetPositionY()));
    Cell cell(p);
    if (dont_load)
        cell.SetNoCreate();
