...
ChannelMgr* channelMgr(Team team)
{
    if (sWorld.getConfig(CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_CHANNEL))
        return &Strawberry::Singleton<AllianceChannelMgr>::Instance();        // cross-faction

    if(team == ALLIANCE)
        return &Strawberry::Singleton<AllianceChannelMgr>::Instance();
    if(team == HORDE)
        return &Strawberry::Singleton<HordeChannelMgr>::Instance();
