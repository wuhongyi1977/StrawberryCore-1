...
class SqlPlainRequest : public SqlOperation
{
    private:
        const char *m_sql;
    public:
        SqlPlainRequest(const char *sql) : m_sql(strawberry_strdup(sql)){}
...
class SqlResultQueue : public ACE_Based::LockedQueue<Strawberry::IQueryCallback* , ACE_Thread_Mutex>
{
    public:
        SqlResultQueue() {}
        void Update();
...
class SqlQuery : public SqlOperation
{
    private:
        const char *m_sql;
        Strawberry::IQueryCallback * m_callback;
        SqlResultQueue * m_queue;
    public:
        SqlQuery(const char *sql, Strawberry::IQueryCallback * callback, SqlResultQueue * queue)
            : m_sql(strawberry_strdup(sql)), m_callback(callback), m_queue(queue) {}
...
class SqlQueryHolder
{
    ...
    public:
        ...
        bool Execute(Strawberry::IQueryCallback * callback, SqlDelayThread *thread, SqlResultQueue *queue);
};
...
class SqlQueryHolderEx : public SqlOperation
{
    private:
        ...
        SqlResultQueue * m_queue;
    public:
        SqlQueryHolderEx(SqlQueryHolder *holder, Strawberry::IQueryCallback * callback, SqlResultQueue * queue)
