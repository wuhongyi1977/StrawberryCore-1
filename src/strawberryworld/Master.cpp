...
int Master::Run()
{
    ...
    // set realmbuilds depend on strawberryworld expected builds, and set server online
    {
...
//        if(Prio && (m_ServiceStatus == -1)/* need set to default process priority class in service mode*/)
        if(Prio)
        {
            if(SetPriorityClass(hProcess,HIGH_PRIORITY_CLASS))
                sLog.outString("strawberryworld process priority class set to HIGH");
            else
                sLog.outError("Can't set strawberryworld process priority class.");
            sLog.outString();
        }
    }
    #endif
